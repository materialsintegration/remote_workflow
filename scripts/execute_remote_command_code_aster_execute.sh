...
eval $ssh_makedir
#eval $ssh_client "if [ -e /tmp/$calc_dir ]; then rm -rf /tmp/$calc_dir; fi; mkdir /tmp/$calc_dir"
# ディレクトリを転送
...
eval $ssh_remotecmd
#ssh $ssh_client "cd /tmp/$calc_dir/codeAstere; $client_cmd > /tmp/$calc_dir/codeAster/remote_program_exec.log 2>&1"
if [ $? != 0 ]; then
