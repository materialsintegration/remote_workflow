...
2. MInt提供側は、下記のセキュリティ対策を実施すること。
    ...
    * MIntシステムを構成する各サーバに対し、定期的に JVN 脆弱性情報を確認すること。
...
* 外部計算機資源において秘匿データを扱うことが可能である

    + MIntシステムからみると、外部計算機はブラックボックスであるため、入出力データ以外は関知できない。
    + そのため、秘匿データへのアクセスを外部計算機内で完結させ、MIntシステムに返却する実行結果に含めないことで、秘匿データを機関外(社外)に持ち出さずに利用できる。
