...
原則としてパスフレーズ無しの公開鍵認証とするため、公開鍵ファイルをMInt側に設定する必要がある。
...
事前に取り決めた一意な(他ユーザと重ならない)「サイト名(半角英数字)」と、MIntのユーザアカウントに対して設定されている「APIアクセストークン」を用意する。
...
2. ワークフローの準備
    ...
    + 1. で作成した予測モジュールを使用するワークフローを作成する。
3. SSHの場合
