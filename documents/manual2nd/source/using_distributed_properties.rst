...
3. 実行するプログラム
    ...
    + 2. の検討の結果、外部計算機資源側で実行するプログラム及び必要なパラメータの調査。
    + MIntシステムから呼び出されるスクリプトの決定。
...
原則としてパスフレーズ無しの公開鍵認証とする。

1. 外部計算機側でRSA公開鍵を用意する。既存の鍵でも可。下記は新規の鍵セットを作成する場合のコマンドである。
...
2. 作成された「~/.ssh/id_rsa.pub」ファイルをMInt側担当者に安全な方法で送付する。

WebAPI方式の認証情報
--------------------

MInt運用担当に問い合わせ、一意な(他ユーザと重ならない)「サイト名(半角英数字)」と、MIntのユーザアカウントに対して設定されている「APIアクセストークン」を用意する。

* ホスト情報

    + MInt側でAPI問い合わせに対する個別の識別を行うためにサイト情報（文字列として区別できれば何でもよい）が必要である。
* APIトークン
