...
MIntには、ワークフローを構成するモジュール内の一部分の処理を、MIntの計算ノード以外の「外部計算機」に行わせる機能がある。本機能によって、ユーザ(★「企業ユーザ」と限定することもできるが、大学などでも外部資源利用はありえるので限定する文言が思いつかない。どうすれば良いか？★)には下記に挙げる利点がある。
...
外部計算資源の利用イメージを下図(★図番号を適切に参照する方法が分からない。間中さん対応お願いします。以下同様★)に示す。
...
SSH 方式での外部資源利用のイメージを下図(★図番号★)に示す。
...
(★(源)このモジュールが外部計算で扱うものという記載が最初に欲しいです→(伊藤)本図は図2.1とかぶるし、説明は以降の章に出てくるので節自体が不要かと考えている。また、本図に書かれたモジュールは全て外部計算になっている。★)
...
WebAPI方式での外部計算の実行イメージを下図(★図番号★)に示す。
...
(★(伊藤)このあたり、SSH方式のように細かい説明が書かれていないので、書きぶりを合わせる必要がある★)
...
(★(源)ユーザが何もない状態から気軽に試してみたい場合のサンプルになっていますか？ライセンスが関係ない、計算時間が短い、転送量が少ない、などで動作確認をするレベルのサンプルのことです。abaqusのライセンスを持っていないユーザはテストできない？→→(伊藤)高速疲労予測なのでAbaqusではなくMathematicaかと思いますが、いずれにせよ商用ソフトが不要なワークフローをサンプルにすべきかと思います★)
...
ユーザがワークフローの廃止届(★(源)書式があるのですか？通常の権限操作と同じポリシーですか？復活は可能ですか？★★)を提出する。当該ワークフローはMInt上で「無効」のステータスを付与され参照・実行不能となる。
